...
REM Program sends an email whenever utilization data exceeds
REM  the disk space utilization threshold
REM Scheduled to run in the afternoon MNL time
...
:MAIN
  ...
  ECHO ERRORFLAG to Send Email: %ERRORFLAG%
  
  CALL:CreateEmail
  
  IF %ERRORFLAG% GEQ 1 CALL:SendEmail
:END
ENDLOCAL
GOTO:EOF
...
:ProcessMemoryInfo
  ...
  IF %procFreeMem% LEQ %lowThresholdMem% (
    REM Commented line below for Afternoon Monitoring
    REM SET ERRORFLAG=1
...
:CreateEmail
  ...
  FOR /F %%a IN ('HOSTNAME') DO SET myHostName=%%a
  
  REM Email Subject
  ECHO ALERT -- Windows %myHostName% -- Afternoon Server Monitoring >> "%tempDir%\email_subj.txt"
